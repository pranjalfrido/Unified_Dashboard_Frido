                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, flex: 1 }}>
                    {[
                      ...
                      { label: 'Return% (SC)', value: returnRateReliable ? `${(amzSC.returnRate?.pct || 0).toFixed(1)}%` : 'N/A', sub: returnRateReliable ? `${fmt(amzSC.returnRate?.rollReturned || 0)} of ${fmt(amzSC.returnRate?.rollOrders || 0)} exc GST` : 'No reliable data', accent: returnRateReliable && (amzSC.returnRate?.pct || 0) > 18 ? '#7A1A1A' : undefined },
                    ].map(k => (
                      <div key={k.label} className="kpi-card" style={{ padding: '10px 13px' }}>
                        <div className="kpi-label">{k.label}</div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 4 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 5fr', gap: 10, alignItems: 'stretch' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gridAutoRows: '1fr', gap: 10 }}>
                  {[
                    ...
                    { label: 'Return Rate', value: returnRateReliable ? `${(amzSC.returnRate?.pct || 0).toFixed(1)}%` : 'N/A', sub: returnRateReliable ? `${fmt(amzSC.returnRate?.rollReturned || 0)} of ${fmt(amzSC.returnRate?.rollOrders || 0)} exc GST` : 'No reliable data', accent: returnRateReliable && (amzSC.returnRate?.pct || 0) > 18 ? '#7A1A1A' : undefined },
                  ].map(k => (
                    <div key={k.label} className="kpi-card" style={{ padding: '10px 13px' }}>
                      <div className="kpi-label">{k.label}</div>
