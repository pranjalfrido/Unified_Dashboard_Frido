          ...
          <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column' }}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <ComposedChart data={tatData} margin={{ top: 10, right: -20, left: -30, bottom: 0 }}>
                ...
                <Line yAxisId="right" type="monotone" dataKey="avg_processing_days" name="Avg Processing Days" stroke="#333" strokeWidth={1.5} dot={false} />
                <Line yAxisId="right" type="monotone" dataKey="avg_pickup_days" name="Avg Pickup Days" stroke="#555" strokeWidth={1.5} dot={false} />
                <Line yAxisId="right" type="monotone" dataKey="avg_intransit_days" name="Avg Intransit Days" stroke="#777" strokeWidth={1.5} dot={false} />
                <Line yAxisId="right" type="monotone" dataKey="avg_fulfilment_days" name="Avg Fulfilment Days" stroke="#999" strokeWidth={2} dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
        ...
        return (
          <>
            ...
            {(() => {
              ...
              const BOX_H = 320
              const thS = { fontSize: 12.5, fontWeight: 700, color: C.t3, textTransform: 'uppercase', letterSpacing: '.04em', padding: '9px 10px', borderBottom: `1.5px solid ${C.border}`, whiteSpace: 'nowrap', textAlign: 'right', background: C.card }
              const thL = { ...thS, textAlign: 'left' }
              const tdS = { fontSize: 14.4, color: C.t2, padding: '5.75px 10px', borderBottom: `1px solid ${C.border}`, textAlign: 'right', whiteSpace: 'nowrap' }
              const tdL = { ...tdS, textAlign: 'left', fontWeight: 600, color: C.t1 }
              const totalRowS = { fontSize: 14.4, fontWeight: 700, color: C.t1, padding: '9px 10px', textAlign: 'right', whiteSpace: 'nowrap', background: C.bg, borderTop: `2px solid ${C.border}` }
              ...
              return (
                <>
                  ...
                  {!secCollapsed['tatbucket'] && (
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: -6 }}>
                      <div style={{ display: 'inline-flex', border: `1px solid ${C.border2}`, borderRadius: 6, overflow: 'hidden' }}>
                        {['% Share','Count'].map((opt,i) => {
                          const val = opt === '% Share' ? 'pct' : 'count'
                          return (
                            <button key={opt} onClick={() => setTatMode(val)} style={{
                              padding: '3px 10px', fontSize: 9.5, fontWeight: tatMode === val ? 700 : 400,
                              background: tatMode === val ? C.t1 : 'transparent',
                              color: tatMode === val ? '#fff' : C.t2,
                              border: 'none', cursor: 'pointer', fontFamily: 'var(--font)',
                              borderLeft: i > 0 ? `1px solid ${C.border2}` : 'none'
                            }}>{opt}</button>
                          )
                  ...
                  <div style={{ display: secCollapsed['tatbucket'] ? 'none' : 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 14 }}>
                    ...
                    <div style={{ ...tableCard2, height: 350 }}>
                      <div style={{ ...tableTitle2, fontSize: 12.4 }}>Order Processing Time <span style={{ fontWeight: 500, color: C.t3, fontSize: 11.9, marginLeft: 4 }}>(by Facility)</span></div>
                    ...
                    <div style={{ ...tableCard2, height: 350 }}>
                      <div style={{ ...tableTitle2, fontSize: 12.4 }}>Order Pickup Time <span style={{ fontWeight: 500, color: C.t3, fontSize: 11.9, marginLeft: 4 }}>(by Courier)</span></div>
                      <div style={{ margin: '0 8px', flex: 1, overflowY: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                          <colgroup>{cw.map((w,i)=><col key={i} style={{width:w}}/>)}</colgroup>
                          <thead><tr style={{ background: C.bg }}>
                    ...
                    <div style={{ ...tableCard2, height: 350 }}>
                      <div style={{ ...tableTitle2, fontSize: 12.4 }}>In-Transit Time <span style={{ fontWeight: 500, color: C.t3, fontSize: 11.9, marginLeft: 4 }}>(by Courier)</span></div>
                      <div style={{ margin: '0 8px', flex: 1, overflowY: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                    ...
                    <div style={{ ...tableCard2, height: 350 }}>
                      <div style={{ ...tableTitle2, fontSize: 12.4 }}>Fulfilment Time <span style={{ fontWeight: 500, color: C.t3, fontSize: 11.9, marginLeft: 4 }}>(by Facility)</span></div>
                      <div style={{ margin: '0 8px', flex: 1, overflowY: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                          <colgroup>{cw.map((w,i)=><col key={i} style={{width:w}}/>)}</colgroup>
                          <thead><tr style={{ background: C.bg }}>
