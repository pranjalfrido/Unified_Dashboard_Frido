      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '14px 16px', flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
          {[
            ...
            { label: 'Orders', value: fmtN((() => { if (!selPlatform) return data.nOrders || 0; if (selPlatform === 'Amazon') return data.amzSC?.totalOrders || 0; if (selPlatform === 'Flipkart') return (data.flipkart?.totals || []).reduce((s, t) => s + (t.orders || 0), 0); if (selPlatform === 'Myntra') return data.myntra?.totals?.orders || 0; if (selPlatform === 'Zepto') return data.zepto?.totals?.orders || 0; if (selPlatform === 'Instamart') return data.instamart?.totals?.orders || 0; if (selPlatform === 'Blinkit') return data.blinkit?.totals?.orders || 0; if (selPlatform === 'Meta' || selPlatform === 'Google') return data.shopify?.totals?.orders || 0; return 0 })()), sub: 'Distinct orders' },
          ].map(k => (
