              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {['Daily','Weekly','Monthly'].map(g => (
                    <button key={g} onClick={() => setTrendGranularity(g)} style={{ fontSize: 11, padding: '3px 10px', borderRadius: 6, border: `1px solid ${trendGranularity === g ? C.acc : C.border}`, background: trendGranularity === g ? C.acc : C.card, color: trendGranularity === g ? '#000' : C.t2, cursor: 'pointer', fontWeight: trendGranularity === g ? 700 : 500, fontFamily: 'var(--font)' }}>{g}</button>
                  ))}
                </div>
              </div>
            </div>
