...
export default async function inventoryHandler(req, res) {
  ...
  try {
    const [invRows, salesRows, lastSaleRows, itemMasterRows, skuMappingRows, shopifyInvRows, facilityRows, regionRows, channelRows] = await Promise.all([
      cachedQuery('inv',     TTL_1H,  async () => { const { rows } = await db.query(`SELECT item_sku_code AS "ItemSkuCode", facility AS "Facility", updated AS "Updated", inventory AS "Inventory", inventory_blocked AS "InventoryBlocked", rtd_invt AS "RtdInvt", raw_invt AS "RawInvt", raw_blocked_invt AS "RawBlockedInvt" FROM inv_snapshot`); return rows }),
