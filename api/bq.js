  ...
  try {
    ...
    const payload = {
      ...
      ads: {
        ...
        categoryBreakdown: (r.adsCategoryBreakdown || []).map(x => ({ platform: x.platform, targetType: x.target_type, category: x.category || 'Unknown', subCategory: x.target_type === 'product' ? x.product_name : null, spend: parseFloat(x.spend)||0, orders: parseFloat(x.orders)||0, revenue: parseFloat(x.revenue)||0, clicks: parseFloat(x.clicks)||0, impressions: parseFloat(x.impressions)||0, ctr: parseFloat(x.ctr)||0, cpc: parseFloat(x.cpc)||0, roas: parseFloat(x.roas)||0 })),
