...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    fkSKUs: `SELECT TRIM(REPLACE(REGEXP_REPLACE(sku,r'"{2,}SKU:-*"{0,}',''),'"','')) AS sku, CASE WHEN UPPER(TRIM(fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT order_id) AS orders, SUM(CAST(item_quantity AS INT64)) AS units, ROUND(SUM(CAST(price_after_discount_price_before_discount_total_discountx AS FLOAT64)),0) AS rev FROM \`frido-429506.flipkart_reports.sales_report\` WHERE DATE(SUBSTR(order_date,1,10)) BETWEEN '${start}' AND '${end}' AND sku IS NOT NULL GROUP BY sku, sub ORDER BY rev DESC LIMIT 30`,
