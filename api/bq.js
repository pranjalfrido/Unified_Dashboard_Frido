...
ORDER BY order_date`,
    ...
    offlineTotals: `WITH q AS (${base}) SELECT SubChannel, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' GROUP BY SubChannel`,
    prevOffline: `WITH q AS (${prevBase}) SELECT SubChannel, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' GROUP BY SubChannel`,
    prevOfflineDaily: `WITH q AS (${prevBase}) SELECT CAST(OrderDate AS STRING) AS date, SubChannel, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='offline_sales' GROUP BY date, SubChannel ORDER BY date`,
    offlineDaily: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, SubChannel, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(ItemQty) AS units, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev FROM q WHERE Channel='offline_sales' GROUP BY date, SubChannel ORDER BY date`,
    offlineSubChannel: `WITH q AS (${base}) SELECT SubChannel, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(ItemQty) AS units, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev FROM q WHERE Channel='offline_sales' GROUP BY SubChannel ORDER BY rev DESC`,
    offlineCategory: `WITH q AS (${base}) SELECT SubChannel, Category, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' GROUP BY SubChannel, Category ORDER BY rev DESC`,
    offlineSubCategory: `WITH q AS (${base}) SELECT SubChannel, Category, SubCategory, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' GROUP BY SubChannel, Category ORDER BY rev DESC`,
    offlineSKU: `WITH q AS (${base}) SELECT SubChannel, Category, SubCategory, MasterSKU AS sku, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' AND MasterSKU IS NOT NULL AND TRIM(MasterSKU) != '' GROUP BY SubChannel, Category, SubCategory, MasterSKU ORDER BY rev DESC LIMIT 600`,
    offlineState: `WITH q AS (${base}) SELECT SubChannel, UPPER(TRIM(State)) AS state, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT City) AS cities FROM q WHERE Channel='offline_sales' AND State IS NOT NULL AND TRIM(State) != '' GROUP BY SubChannel, UPPER(TRIM(State)) ORDER BY rev DESC`,
    offlineCity: `WITH q AS (${base}) SELECT SubChannel, INITCAP(TRIM(City)) AS city, INITCAP(TRIM(State)) AS state, MAX(Region) AS region, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='offline_sales' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY SubChannel, INITCAP(TRIM(City)), INITCAP(TRIM(State)) ORDER BY rev DESC LIMIT 200`,
    offlineRegion: `WITH q AS (${base}) SELECT SubChannel, Region AS region, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' AND Region IS NOT NULL GROUP BY SubChannel, Region ORDER BY rev DESC`,
    offlineTier: `WITH q AS (${base}) SELECT SubChannel, City_Tier AS city_tier, Tier_Label AS tier_label, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Channel='offline_sales' AND City_Tier IS NOT NULL GROUP BY SubChannel, City_Tier, Tier_Label ORDER BY City_Tier`,
  }
  ...
  try {
    ...
    const payload = {
      ...
      offline: {
        totalsBySub: (r.offlineTotals || []).map(x => ({ subChannel: x.SubChannel, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0 })),
        prevBySub: (r.prevOffline || []).map(x => ({ subChannel: x.SubChannel, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0 })),
        ...
        categoryRows: (r.offlineCategory || []).map(x => ({ subChannel: x.SubChannel, category: x.Category || 'Unknown', rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0 })),
