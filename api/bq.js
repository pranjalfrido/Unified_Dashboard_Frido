...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    amzSCReturnRate: `WITH q AS (${base}) SELECT ROUND(SUM(CASE WHEN Order_Status != 'Return' AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Inc_GST ELSE 0 END), 2) AS total_rev_inc, ROUND(SUM(CASE WHEN Order_Status = 'Return' THEN ABS(SellingPrice_Inc_GST) ELSE 0 END), 2) AS returned_rev FROM q WHERE SubChannel = 'Amazon Seller Central'`,
