...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    adsCategoryBreakdown: `SELECT a.platform, a.target_type, CASE WHEN a.target_type = 'product' THEN COALESCE(MAX(i_prod.Category_Name), 'Unknown') WHEN a.target_type = 'category' THEN COALESCE(MAX(i_cat.Category_Name), a.product_name) END AS category, CASE WHEN a.target_type = 'product' THEN a.product_name ELSE NULL END AS sub_category, ROUND(SUM(a.spend),0) AS spend, ROUND(SUM(a.orders),0) AS orders, ROUND(SUM(a.revenue),0) AS revenue, ROUND(SUM(a.clicks),0) AS clicks, ROUND(SUM(a.impressions),0) AS impressions, ROUND(SAFE_DIVIDE(SUM(a.clicks),SUM(a.impressions))*100,2) AS ctr, ROUND(SAFE_DIVIDE(SUM(a.spend),SUM(a.clicks)),2) AS cpc, ROUND(SAFE_DIVIDE(SUM(a.revenue),SUM(a.spend)),2) AS roas FROM \`frido-429506.production.fact_all_platform_ads_report\` a LEFT JOIN \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` i_prod ON a.target_type = 'product' AND LOWER(TRIM(a.product_name)) = LOWER(TRIM(i_prod.Sub_category)) LEFT JOIN \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` i_cat ON a.target_type = 'category' AND LOWER(TRIM(a.product_name)) = LOWER(TRIM(i_cat.Category_Name)) WHERE a.report_date BETWEEN '${start}' AND '${end}' AND a.target_type != 'all' AND a.product_name IS NOT NULL AND TRIM(a.product_name) != '' GROUP BY a.platform, a.target_type, sub_category ORDER BY a.platform, spend DESC LIMIT 500`,
  ...
  try {
    ...
    const payload = {
      ...
      ads: {
        ...
        categoryBreakdown: (r.adsCategoryBreakdown || []).map(x => ({ platform: x.platform, targetType: x.target_type, category: x.category || 'Unknown', subCategory: x.sub_category || null, spend: parseFloat(x.spend)||0, orders: parseFloat(x.orders)||0, revenue: parseFloat(x.revenue)||0, clicks: parseFloat(x.clicks)||0, impressions: parseFloat(x.impressions)||0, ctr: parseFloat(x.ctr)||0, cpc: parseFloat(x.cpc)||0, roas: parseFloat(x.roas)||0 })),
