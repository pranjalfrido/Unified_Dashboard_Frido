...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    byChannel: `WITH q AS (${base}) SELECT Channel, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS qty FROM q WHERE NOT (Channel = 'Shopify' AND SubChannel = 'Shopify B2B') GROUP BY Channel ORDER BY rev DESC`,
    ...
    prevByChannel: `WITH q AS (${prevBase}) SELECT Channel, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev FROM q WHERE NOT (Channel = 'Shopify' AND SubChannel = 'Shopify B2B') GROUP BY Channel`,
  ...
  try {
    ...
    const payload = {
      ...
      prevChMap: Object.fromEntries((r.prevByChannel || []).map(x => [x.Channel, { rev: parseFloat(x.rev) || 0, excRev: parseFloat(x.exc_rev) || 0 }])),
