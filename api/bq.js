...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shCity: `WITH q AS (${base}) SELECT INITCAP(UPPER(TRIM(City_L3))) AS city, UPPER(TRIM(State)) AS state, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND City_L3 IS NOT NULL AND TRIM(City_L3) != '' GROUP BY INITCAP(UPPER(TRIM(City_L3))), UPPER(TRIM(State)) ORDER BY rev DESC LIMIT 50`,
    ...
    byCity: `WITH q AS (${base}) SELECT INITCAP(UPPER(TRIM(City_L3))) AS city, UPPER(TRIM(State)) AS state, Region AS region, City_Tier AS city_tier, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE City_L3 IS NOT NULL AND TRIM(City_L3) != '' GROUP BY INITCAP(UPPER(TRIM(City_L3))), UPPER(TRIM(State)), Region, City_Tier ORDER BY rev DESC LIMIT 50`,
    ...
    prevBl: `WITH q AS (${prevBase}) SELECT SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units, COUNT(DISTINCT ChannelSKUCode) AS skus, COUNT(DISTINCT City) AS cities FROM q WHERE Channel='Blinkit'`,
    ...
    prevCr: `WITH q AS (${prevBase}) SELECT SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units, COUNT(DISTINCT ChannelSKUCode) AS skus, COUNT(DISTINCT City) AS cities FROM q WHERE Channel='CRED'`,
    ...
    fkDaily: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, CASE WHEN SubChannel='Flipkart FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev FROM q WHERE Channel='Flipkart' GROUP BY date, sub ORDER BY date`,
    ...
    blTotals: `WITH q AS (${base}) SELECT SUM(ItemQty) AS units, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev, ROUND(SUM(SellingPrice_Exc_GST),0) AS exc_rev, COUNT(DISTINCT ChannelSKUCode) AS skus, COUNT(DISTINCT City) AS cities, COUNT(DISTINCT OrderDate) AS days FROM q WHERE Channel='Blinkit'`,
    ...
    crDaily: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev FROM q WHERE Channel='CRED' GROUP BY date ORDER BY date`,
  ...
  try {
    ...
    const payload = {
      ...
      cred: {
        ...
        daily: (r.crDaily || []).map(x => ({ date: x.date, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, rev: parseFloat(x.rev)||0 })),
