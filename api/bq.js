...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shCity: `WITH q AS (${base}) SELECT UPPER(TRIM(City_L3)) AS city, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND City_L3 IS NOT NULL AND TRIM(City_L3) != '' GROUP BY UPPER(TRIM(City_L3)) ORDER BY rev DESC LIMIT 50`,
    ...
    byCity: `WITH q AS (${base}) SELECT UPPER(TRIM(City_L3)) AS city, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE City_L3 IS NOT NULL AND TRIM(City_L3) != '' GROUP BY UPPER(TRIM(City_L3)) ORDER BY rev DESC LIMIT 50`,
