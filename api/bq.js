...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    byState: `WITH q AS (${base}) SELECT State AS state, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT City) AS cities FROM q WHERE State IS NOT NULL AND TRIM(State) != '' GROUP BY State ORDER BY rev DESC LIMIT 30`,
    byRegion: `WITH q AS (${base}) SELECT Region AS region, City_Tier AS city_tier, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Region IS NOT NULL GROUP BY Region, City_Tier ORDER BY rev DESC`,
    ...
    byCity: `WITH q AS (${base}) SELECT City AS city, State AS state, Region AS region, City_Tier AS city_tier, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE City IS NOT NULL AND TRIM(City) != '' GROUP BY City, State, Region, City_Tier ORDER BY rev DESC LIMIT 50`,
  ...
  try {
    ...
    const regionRows = (r.byRegion || []).map(x => ({ region: x.region, cityTier: x.city_tier, orders: parseInt(x.orders) || 0, rev: parseFloat(x.rev) || 0, units: parseInt(x.units) || 0 }))
    ...
    const payload = {
      ...
      flipkart: {
        ...
        daily: (r.fkDaily || []).map(x => ({ date: x.date, sub: x.sub, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, rev: parseFloat(x.rev)||0 })),
