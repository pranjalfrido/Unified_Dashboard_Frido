...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shNetCalc: subChannel === 'International'
      ? (() => { const cWhere = country ? ` AND source_system = '${country.replace(/'/g,"''")}'` : ''; return `SELECT SUM(final_total_incl_tax) AS gross, SUM(total_excl_tax) AS exc_rev, 0 AS cancel_rev, 0 AS rto_rev, 0 AS return_rev, 0 AS cir_rev FROM \`frido-429506.production.fact_shopify_international_orders\` WHERE order_date BETWEEN '${start}' AND '${end}' AND (financial_status IS NULL OR financial_status != 'voided')${cWhere}` })()
      : `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS gross, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status='RTO' THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='Return' THEN SellingPrice_Inc_GST ELSE 0 END) AS return_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev FROM q WHERE Channel='Shopify' AND SubChannel != 'Shopify International' AND SubChannel != 'Retail Store'`,
    prevShNetCalc: subChannel === 'International'
      ? (() => { const cWhere = country ? ` AND source_system = '${country.replace(/'/g,"''")}'` : ''; return `SELECT SUM(final_total_incl_tax) AS gross, SUM(total_excl_tax) AS exc_rev, 0 AS cancel_rev, 0 AS rto_rev, 0 AS return_rev, 0 AS cir_rev FROM \`frido-429506.production.fact_shopify_international_orders\` WHERE order_date BETWEEN '${ps}' AND '${pe}' AND (financial_status IS NULL OR financial_status != 'voided')${cWhere}` })()
    ...
    shCategoryPrev: `WITH q AS (${prevBase}) SELECT Category, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' GROUP BY Category`,
    shSubCategory: `WITH q AS (${base}) SELECT Category, SubCategory, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units, SUM(CASE WHEN UPPER(COALESCE(MasterSKU,'')) NOT LIKE '%COUP%' AND UPPER(COALESCE(MasterSKU,'')) NOT LIKE '%DFA%' THEN ItemQty ELSE 0 END) AS asp_units, COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancelled, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return') THEN OrderId END) AS rto, COUNT(DISTINCT CASE WHEN Order_Status='CIR' THEN OrderId END) AS cir, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exch, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status IN ('RTO','Return') THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev, SUM(CASE WHEN Order_Status='Exchange' THEN SellingPrice_Inc_GST ELSE 0 END) AS exch_rev FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' GROUP BY Category, SubCategory ORDER BY rev DESC`,
    shSubCategoryPrev: `WITH q AS (${prevBase}) SELECT Category, SubCategory, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' GROUP BY Category, SubCategory`,
    shSKU: `WITH q AS (${base}) SELECT Category, SubCategory, MasterSKU AS sku, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units, COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancelled, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return') THEN OrderId END) AS rto, COUNT(DISTINCT CASE WHEN Order_Status='CIR' THEN OrderId END) AS cir, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exch, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status IN ('RTO','Return') THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev, SUM(CASE WHEN Order_Status='Exchange' THEN SellingPrice_Inc_GST ELSE 0 END) AS exch_rev FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND MasterSKU IS NOT NULL AND TRIM(MasterSKU) != '' GROUP BY Category, SubCategory, MasterSKU ORDER BY rev DESC LIMIT 300`,
    shSKUPrev: `WITH q AS (${prevBase}) SELECT Category, SubCategory, MasterSKU AS sku, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND MasterSKU IS NOT NULL AND TRIM(MasterSKU) != '' GROUP BY Category, SubCategory, MasterSKU`,
    shState: `WITH q AS (${base}) SELECT UPPER(TRIM(State)) AS state, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units, COUNT(DISTINCT City) AS cities, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return','CIR') THEN OrderId END) AS rto_orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND State IS NOT NULL AND TRIM(State) != '' GROUP BY UPPER(TRIM(State)) ORDER BY rev DESC LIMIT 30`,
    shStateTotal: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS total_rev, COUNT(DISTINCT OrderId) AS total_orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND State IS NOT NULL AND TRIM(State) != ''`,
    shStatePrev: `WITH q AS (${prevBase}) SELECT UPPER(TRIM(State)) AS state, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT OrderId) AS orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND State IS NOT NULL AND TRIM(State) != '' GROUP BY UPPER(TRIM(State))`,
    shRegion: `WITH q AS (${base}) SELECT Region AS region, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND Region IS NOT NULL GROUP BY Region ORDER BY rev DESC`,
    shTier: `WITH q AS (${base}) SELECT City_Tier AS city_tier, Tier_Label AS tier_label, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND City_Tier IS NOT NULL GROUP BY City_Tier, Tier_Label ORDER BY City_Tier`,
    shCity: `WITH q AS (${base}) SELECT INITCAP(TRIM(City)) AS city, INITCAP(TRIM(State)) AS state, MAX(Region) AS region, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return','CIR') THEN OrderId END) AS rto_orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY INITCAP(TRIM(City)), INITCAP(TRIM(State)) ORDER BY rev DESC LIMIT 50`,
    shCityTotal: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS total_rev, COUNT(DISTINCT OrderId) AS total_orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND City IS NOT NULL AND TRIM(City) != ''`,
    shCityPrev: `WITH q AS (${prevBase}) SELECT INITCAP(TRIM(City)) AS city, INITCAP(TRIM(State)) AS state, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT OrderId) AS orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY INITCAP(TRIM(City)), INITCAP(TRIM(State))`,
    ...
    byRefundTrend: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, COUNT(DISTINCT OrderId) AS total_orders, COUNTIF(RefundStatus = 'true') AS refund_lines FROM q WHERE Channel = 'Shopify' AND SubChannel != 'Retail Store' GROUP BY date ORDER BY date`,
    ...
    prevShopify: subChannel === 'International'
      ...
      : `WITH q AS (${prevBase}) SELECT SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return') THEN OrderId END) AS rto_orders, COUNT(DISTINCT CASE WHEN Order_Status = 'CIR' THEN OrderId END) AS cir_orders, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exchange_orders FROM q WHERE Channel='Shopify' AND SubChannel NOT IN ('Shopify International', 'Retail Store')`,
    ...
    shDaily: subChannel === 'International'
      ...
      : `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND SubChannel NOT IN ('Shopify International', 'Retail Store') GROUP BY date ORDER BY date`,
    prevShopifyDaily: subChannel === 'International'
      ...
      : `WITH q AS (${prevBase}) SELECT CAST(OrderDate AS STRING) AS date, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel NOT IN ('Shopify International', 'Retail Store') GROUP BY date ORDER BY date`,
    prevShopifyCancel: `WITH q AS (${prevBase}) SELECT COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancelled_orders, COUNT(DISTINCT OrderId) AS total_orders FROM q WHERE Channel='Shopify' AND SubChannel != 'Retail Store'`,
    ...
    amzSCNetCalc: `WITH q AS (${base}) SELECT ROUND(SUM(CASE WHEN FulfilmentStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Inc_GST ELSE 0 END),2) AS gross_rev, ROUND(SUM(CASE WHEN FulfilmentStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Exc_GST ELSE 0 END),2) AS gross_exc_rev, ROUND(SUM(CASE WHEN Order_Status IN ('Return','RTO') THEN ABS(SellingPrice_Inc_GST) ELSE 0 END),2) AS return_rev, ROUND(SUM(CASE WHEN FinancialStatus = 'Cancelled' THEN ABS(SellingPrice_Inc_GST) ELSE 0 END),2) AS cancel_rev FROM q WHERE SubChannel = 'Amazon Seller Central'`,
    ...
    amzSCStates: `WITH q AS (${base}), base AS (SELECT UPPER(TRIM(State)) AS ship_state, OrderId, Order_Status, SellingPrice_Inc_GST, FulfilmentStatus FROM q WHERE SubChannel = 'Amazon Seller Central') SELECT ship_state, COUNT(DISTINCT CASE WHEN FulfilmentStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN OrderId END) AS orders, ROUND(SUM(CASE WHEN FulfilmentStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Inc_GST ELSE 0 END),0) AS rev, COUNT(DISTINCT CASE WHEN Order_Status IN ('Return','RTO') THEN OrderId END) AS rto_orders FROM base GROUP BY ship_state ORDER BY rev DESC`,
    amzSCCities: `WITH q AS (${base}), base AS (SELECT UPPER(TRIM(City_L2)) AS city, OrderId, Order_Status, SellingPrice_Inc_GST, FinancialStatus FROM q WHERE SubChannel = 'Amazon Seller Central' AND City_L2 IS NOT NULL AND TRIM(City_L2) != '') SELECT city, COUNT(DISTINCT CASE WHEN FinancialStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN OrderId END) AS orders, ROUND(SUM(CASE WHEN FinancialStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Inc_GST ELSE 0 END),0) AS rev, COUNT(DISTINCT CASE WHEN Order_Status IN ('Return','RTO') THEN OrderId END) AS rto_orders FROM base GROUP BY city ORDER BY rev DESC LIMIT 50`,
    ...
    crCities: `WITH q AS (${base}) SELECT INITCAP(TRIM(City)) AS city, COUNT(DISTINCT OrderId) AS orders, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev FROM q WHERE Channel='CRED' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY city ORDER BY rev DESC`,
    ...
    mnCities: `WITH q AS (${base}) SELECT City AS city, Region AS region, City_Tier AS city_tier, COUNT(DISTINCT OrderId) AS orders, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev FROM q WHERE Channel='Myntra' AND City IS NOT NULL GROUP BY city, region, city_tier ORDER BY rev DESC`,
    ...
    offlineCategory: `WITH q AS (${base}) SELECT SubChannel, Category, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(CASE WHEN Order_Status='Sales' THEN SellingPrice_Inc_GST ELSE 0 END) AS rev, SUM(CASE WHEN Order_Status='Sales' THEN SellingPrice_Exc_GST ELSE 0 END) AS exc_rev, SUM(CASE WHEN Order_Status='Sales' THEN ItemQty ELSE 0 END) AS units, SUM(CASE WHEN Order_Status='Credit Note' THEN SellingPrice_Inc_GST ELSE 0 END) AS cn_rev FROM q WHERE Channel='offline_sales' GROUP BY SubChannel, Category ORDER BY rev DESC`,
    ...
    offlineRegion: `WITH q AS (${base}) SELECT SubChannel, Region AS region, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(CASE WHEN Order_Status='Sales' THEN SellingPrice_Inc_GST ELSE 0 END) AS rev, SUM(CASE WHEN Order_Status='Sales' THEN ItemQty ELSE 0 END) AS units FROM q WHERE Channel='offline_sales' AND Region IS NOT NULL AND Order_Status='Sales' GROUP BY SubChannel, Region ORDER BY rev DESC`,
    offlineTier: `WITH q AS (${base}) SELECT SubChannel, City_Tier AS city_tier, Tier_Label AS tier_label, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(CASE WHEN Order_Status='Sales' THEN SellingPrice_Inc_GST ELSE 0 END) AS rev, SUM(CASE WHEN Order_Status='Sales' THEN ItemQty ELSE 0 END) AS units FROM q WHERE Channel='offline_sales' AND City_Tier IS NOT NULL AND Order_Status='Sales' GROUP BY SubChannel, City_Tier, Tier_Label ORDER BY City_Tier`,
    ...
    adsCategoryBreakdown: `WITH im_sub AS (SELECT LOWER(TRIM(Sub_category)) AS sub_key, ANY_VALUE(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE Sub_category IS NOT NULL GROUP BY sub_key), im_cat AS (SELECT LOWER(TRIM(Category_Name)) AS cat_key, ANY_VALUE(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE Category_Name IS NOT NULL GROUP BY cat_key), camp_map AS (SELECT LOWER(TRIM(campaign_name)) AS camp_key, ANY_VALUE(product_name) AS product_name, ANY_VALUE(category) AS category, ANY_VALUE(target_type) AS target_type FROM \`frido-429506.ads_campaign_mapping.master_campaign_mapping_sheet_all_platforms\` WHERE campaign_name IS NOT NULL GROUP BY camp_key), mapped AS (SELECT a.platform, COALESCE(m.target_type, a.target_type) AS target_type, COALESCE(m.product_name, a.product_name) AS product_name, COALESCE(m.category, CASE WHEN a.target_type = 'product' THEN COALESCE(i_prod.Category_Name, 'Unknown') WHEN a.target_type = 'category' THEN COALESCE(i_cat.Category_Name, a.product_name) ELSE 'Unknown' END) AS category, a.spend, a.orders, a.revenue, a.clicks, a.impressions FROM \`frido-429506.production.fact_all_platform_ads_report\` a LEFT JOIN camp_map m ON LOWER(TRIM(a.campaign_name)) = m.camp_key AND (a.product_name IS NULL OR TRIM(a.product_name) = '') LEFT JOIN im_sub i_prod ON a.target_type = 'product' AND LOWER(TRIM(a.product_name)) = i_prod.sub_key LEFT JOIN im_cat i_cat ON a.target_type = 'category' AND LOWER(TRIM(a.product_name)) = i_cat.cat_key WHERE a.report_date BETWEEN '${start}' AND '${end}' AND a.target_type != 'all' AND (a.product_name IS NOT NULL AND TRIM(a.product_name) != '' OR m.product_name IS NOT NULL)) SELECT platform, target_type, product_name, category, ROUND(SUM(spend),0) AS spend, ROUND(SUM(orders),0) AS orders, ROUND(SUM(revenue),0) AS revenue, ROUND(SUM(clicks),0) AS clicks, ROUND(SUM(impressions),0) AS impressions, ROUND(SAFE_DIVIDE(SUM(clicks),SUM(impressions))*100,2) AS ctr, ROUND(SAFE_DIVIDE(SUM(spend),SUM(clicks)),2) AS cpc, ROUND(SAFE_DIVIDE(SUM(revenue),SUM(spend)),2) AS roas FROM mapped GROUP BY platform, target_type, product_name, category ORDER BY platform, spend DESC LIMIT 500`,
    ...
    prevAdsCategoryBreakdown: `WITH im_sub AS (SELECT LOWER(TRIM(Sub_category)) AS sub_key, ANY_VALUE(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE Sub_category IS NOT NULL GROUP BY sub_key), im_cat AS (SELECT LOWER(TRIM(Category_Name)) AS cat_key, ANY_VALUE(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE Category_Name IS NOT NULL GROUP BY cat_key), camp_map AS (SELECT LOWER(TRIM(campaign_name)) AS camp_key, ANY_VALUE(product_name) AS product_name, ANY_VALUE(category) AS category, ANY_VALUE(target_type) AS target_type FROM \`frido-429506.ads_campaign_mapping.master_campaign_mapping_sheet_all_platforms\` WHERE campaign_name IS NOT NULL GROUP BY camp_key), mapped AS (SELECT a.platform, COALESCE(m.target_type, a.target_type) AS target_type, COALESCE(m.product_name, a.product_name) AS product_name, COALESCE(m.category, CASE WHEN a.target_type = 'product' THEN COALESCE(i_prod.Category_Name, 'Unknown') WHEN a.target_type = 'category' THEN COALESCE(i_cat.Category_Name, a.product_name) ELSE 'Unknown' END) AS category, a.spend, a.clicks FROM \`frido-429506.production.fact_all_platform_ads_report\` a LEFT JOIN camp_map m ON LOWER(TRIM(a.campaign_name)) = m.camp_key AND (a.product_name IS NULL OR TRIM(a.product_name) = '') LEFT JOIN im_sub i_prod ON a.target_type = 'product' AND LOWER(TRIM(a.product_name)) = i_prod.sub_key LEFT JOIN im_cat i_cat ON a.target_type = 'category' AND LOWER(TRIM(a.product_name)) = i_cat.cat_key WHERE a.report_date BETWEEN '${ps}' AND '${pe}' AND a.target_type != 'all' AND (a.product_name IS NOT NULL AND TRIM(a.product_name) != '' OR m.product_name IS NOT NULL)) SELECT platform, target_type, product_name, category, ROUND(SUM(spend),0) AS spend, ROUND(SUM(clicks),0) AS clicks FROM mapped GROUP BY platform, target_type, product_name, category ORDER BY platform, spend DESC LIMIT 500`,
    shopifyNewCusts: `WITH in_range AS (SELECT DISTINCT customer_id FROM \`frido-429506.production.fact_shopify_myfrido_mobility_all_orders\` WHERE order_date_ist BETWEEN '${start}' AND '${end}' AND customer_id IS NOT NULL), prior AS (SELECT DISTINCT customer_id FROM \`frido-429506.production.fact_shopify_myfrido_mobility_all_orders\` WHERE order_date_ist < '${start}' AND customer_id IS NOT NULL) SELECT COUNT(*) AS n_custs, COUNTIF(p.customer_id IS NOT NULL) AS repeat_custs FROM in_range ir LEFT JOIN prior p USING (customer_id)`,
    eboTotals: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS qty FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store'`,
    eboNetCalc: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS gross, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status='RTO' THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='Return' THEN SellingPrice_Inc_GST ELSE 0 END) AS return_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store'`,
    ...
    eboDaily: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' GROUP BY date ORDER BY date`,
    prevEboDaily: `WITH q AS (${prevBase}) SELECT CAST(OrderDate AS STRING) AS date, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' GROUP BY date ORDER BY date`,
    prevEbo: `WITH q AS (${prevBase}) SELECT SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return') THEN OrderId END) AS rto_orders, COUNT(DISTINCT CASE WHEN Order_Status = 'CIR' THEN OrderId END) AS cir_orders, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exchange_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store'`,
    eboCategory: `WITH q AS (${base}) SELECT Category, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units, SUM(CASE WHEN UPPER(COALESCE(MasterSKU,'')) NOT LIKE '%COUP%' AND UPPER(COALESCE(MasterSKU,'')) NOT LIKE '%DFA%' THEN ItemQty ELSE 0 END) AS asp_units, COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancelled, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return') THEN OrderId END) AS rto, COUNT(DISTINCT CASE WHEN Order_Status='CIR' THEN OrderId END) AS cir, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exch, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status IN ('RTO','Return') THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev, SUM(CASE WHEN Order_Status='Exchange' THEN SellingPrice_Inc_GST ELSE 0 END) AS exch_rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' GROUP BY Category ORDER BY rev DESC`,
    eboCategoryPrev: `WITH q AS (${prevBase}) SELECT Category, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' GROUP BY Category`,
    eboSubCategory: `WITH q AS (${base}) SELECT Category, SubCategory, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units, SUM(CASE WHEN UPPER(COALESCE(MasterSKU,'')) NOT LIKE '%COUP%' AND UPPER(COALESCE(MasterSKU,'')) NOT LIKE '%DFA%' THEN ItemQty ELSE 0 END) AS asp_units, COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancelled, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return') THEN OrderId END) AS rto, COUNT(DISTINCT CASE WHEN Order_Status='CIR' THEN OrderId END) AS cir, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exch, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status IN ('RTO','Return') THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev, SUM(CASE WHEN Order_Status='Exchange' THEN SellingPrice_Inc_GST ELSE 0 END) AS exch_rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' GROUP BY Category, SubCategory ORDER BY rev DESC`,
    eboSubCategoryPrev: `WITH q AS (${prevBase}) SELECT Category, SubCategory, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' GROUP BY Category, SubCategory`,
    ...
    eboStateTotal: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS total_rev, COUNT(DISTINCT OrderId) AS total_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND State IS NOT NULL AND TRIM(State) != ''`,
    eboStatePrev: `WITH q AS (${prevBase}) SELECT UPPER(TRIM(State)) AS state, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT OrderId) AS orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND State IS NOT NULL AND TRIM(State) != '' GROUP BY UPPER(TRIM(State))`,
    eboCity: `WITH q AS (${base}) SELECT INITCAP(TRIM(City)) AS city, INITCAP(TRIM(State)) AS state, MAX(Region) AS region, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT CASE WHEN Order_Status IN ('RTO','Return','CIR') THEN OrderId END) AS rto_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY INITCAP(TRIM(City)), INITCAP(TRIM(State)) ORDER BY rev DESC LIMIT 50`,
    eboCityTotal: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS total_rev, COUNT(DISTINCT OrderId) AS total_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND City IS NOT NULL AND TRIM(City) != ''`,
    eboCityPrev: `WITH q AS (${prevBase}) SELECT INITCAP(TRIM(City)) AS city, INITCAP(TRIM(State)) AS state, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT OrderId) AS orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY INITCAP(TRIM(City)), INITCAP(TRIM(State))`,
    eboRegion: `WITH q AS (${base}) SELECT Region AS region, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND Region IS NOT NULL GROUP BY Region ORDER BY rev DESC`,
    eboTier: `WITH q AS (${base}) SELECT City_Tier AS city_tier, Tier_Label AS tier_label, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND City_Tier IS NOT NULL GROUP BY City_Tier, Tier_Label ORDER BY City_Tier`,
    ...
    eboCIR: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS cir_rev, SUM(SellingPrice_Exc_GST) AS cir_exc_rev, COUNT(DISTINCT OrderId) AS cir_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND Order_Status = 'CIR'`,
    eboReturn: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS return_rev, SUM(SellingPrice_Exc_GST) AS return_exc_rev, COUNT(DISTINCT OrderId) AS return_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND Order_Status = 'Return'`,
    eboExchange: `WITH q AS (${base}) SELECT COUNT(DISTINCT OrderId) AS exchange_orders, SUM(SellingPrice_Inc_GST) AS exchange_rev FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND Order_Status = 'Exchange'`,
    eboRTO: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS rto_rev, SUM(SellingPrice_Exc_GST) AS rto_exc_rev, COUNT(DISTINCT OrderId) AS rto_orders FROM q WHERE Channel='Shopify' AND SubChannel='Retail Store' AND Order_Status = 'RTO'`,
  ...
  try {
    ...
    const catMap = {}
    r.byCategory.forEach(x => { catMap[x.Category || 'Unknown'] = { rev: parseFloat(x.rev) || 0, excRev: parseFloat(x.exc_rev) || 0, orders: { size: parseInt(x.orders) }, units: parseInt(x.units) || 0, aspUnits: parseInt(x.asp_units) || parseInt(x.units) || 0 } })

    const subCatMap = {}
    r.bySubCategory.forEach(x => { const key = `${x.Category || 'Unknown'}::${x.SubCategory || 'Unknown'}`; subCatMap[key] = { rev: parseFloat(x.rev) || 0, orders: { size: parseInt(x.orders) || 0 }, units: parseInt(x.units) || 0, aspUnits: parseInt(x.asp_units) || parseInt(x.units) || 0 } })
    ...
    r.byChannel.forEach(x => {
      const gross = parseFloat(x.rev) || 0
      const excRev = parseFloat(x.exc_rev) || 0
      const cancelRev = parseFloat(x.cancel_rev) || 0
      const rtoRev = parseFloat(x.rto_rev) || 0
      const returnRev = parseFloat(x.return_rev) || 0
      const cirRev = parseFloat(x.cir_rev) || 0
      const grossAfterReturns = gross - cancelRev - rtoRev - returnRev - cirRev
      ...
      if (x.Channel === 'Shopify' && x.SubChannel === 'Retail Store') {
        if (!chMap['EBO']) chMap['EBO'] = { rev: 0, excRev: 0, netRev: 0, orders: 0, qty: 0 }
        chMap['EBO'].rev += gross
        chMap['EBO'].excRev += excRev
        chMap['EBO'].netRev += netRev
        chMap['EBO'].orders += parseInt(x.orders) || 0
        chMap['EBO'].qty += parseInt(x.qty) || 0
      } else {
        const key = x.Channel
        if (!chMap[key]) chMap[key] = { rev: 0, excRev: 0, netRev: 0, orders: 0, qty: 0 }
        chMap[key].rev += gross
        chMap[key].excRev += excRev
        chMap[key].netRev += netRev
    ...
    const returnTrackableRev = parseFloat(t.return_trackable_rev) || 0
    ...
    const payload = {
      ...
      prevChMap: (() => {
        ...
        ;(r.prevByChannel || []).forEach(x => {
          const gross = parseFloat(x.rev) || 0
          const excRev = parseFloat(x.exc_rev) || 0
          const grossAfterReturns = gross - (parseFloat(x.cancel_rev)||0) - (parseFloat(x.rto_rev)||0) - (parseFloat(x.return_rev)||0) - (parseFloat(x.cir_rev)||0)
          const gstRatio = gross > 0 ? (gross - excRev) / gross : 0
          const netRev = grossAfterReturns * (1 - gstRatio)
          if (x.Channel === 'Shopify' && x.SubChannel === 'Retail Store') {
            if (!m['EBO']) m['EBO'] = { rev: 0, excRev: 0, netRev: 0 }
            m['EBO'].rev += gross; m['EBO'].excRev += excRev; m['EBO'].netRev += netRev
          } else {
            const key = x.Channel
            if (!m[key]) m[key] = { rev: 0, excRev: 0, netRev: 0 }
            m[key].rev += gross; m[key].excRev += excRev; m[key].netRev += netRev
          }
        })
      ...
      shopify: {
        ...
        catMap: Object.fromEntries((r.shCategory || []).map(x => [x.Category || 'Unknown', { rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: { size: parseInt(x.orders)||0 }, units: parseInt(x.units)||0, aspUnits: parseInt(x.asp_units)||parseInt(x.units)||0, cancelled: parseInt(x.cancelled)||0, rto: parseInt(x.rto)||0, cir: parseInt(x.cir)||0, exch: parseInt(x.exch)||0, cancelRev: parseFloat(x.cancel_rev)||0, rtoRev: parseFloat(x.rto_rev)||0, cirRev: parseFloat(x.cir_rev)||0, exchRev: parseFloat(x.exch_rev)||0 }])),
        catPrevMap: Object.fromEntries((r.shCategoryPrev || []).map(x => [x.Category || 'Unknown', parseFloat(x.rev)||0])),
        subCatMap: Object.fromEntries((r.shSubCategory || []).map(x => [`${x.Category||'Unknown'}::${x.SubCategory||'Unknown'}`, { rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: { size: parseInt(x.orders)||0 }, units: parseInt(x.units)||0, aspUnits: parseInt(x.asp_units)||parseInt(x.units)||0, cancelled: parseInt(x.cancelled)||0, rto: parseInt(x.rto)||0, cir: parseInt(x.cir)||0, exch: parseInt(x.exch)||0, cancelRev: parseFloat(x.cancel_rev)||0, rtoRev: parseFloat(x.rto_rev)||0, cirRev: parseFloat(x.cir_rev)||0, exchRev: parseFloat(x.exch_rev)||0 }])),
        subCatPrevMap: Object.fromEntries((r.shSubCategoryPrev || []).map(x => [`${x.Category||'Unknown'}::${x.SubCategory||'Unknown'}`, parseFloat(x.rev)||0])),
        ...
        skuPrevMap: (() => { const m = {}; (r.shSKUPrev || []).forEach(x => { const cat = x.Category||'Unknown', sc = x.SubCategory||'Unknown', sku = x.sku; if (!m[cat]) m[cat] = {}; if (!m[cat][sc]) m[cat][sc] = {}; m[cat][sc][sku] = parseFloat(x.rev)||0 }); return m })(),
        stateMap: Object.fromEntries((r.shState || []).filter(x => x.state).map(x => [x.state, { rev: parseFloat(x.rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, cities: { size: parseInt(x.cities)||0 }, rtoOrders: parseInt(x.rto_orders)||0 }])),
        ...
        cityRows: (r.shCity || []).map(x => ({ city: x.city, state: x.state || '', region: x.region || '', orders: parseInt(x.orders)||0, rev: parseFloat(x.rev)||0, rtoOrders: parseInt(x.rto_orders)||0 })).filter(x => x.city),
      ...
      ebo: (() => {
        ...
        const excRev = parseFloat(nc.exc_rev) || 0
        const cancelRev = parseFloat(nc.cancel_rev) || 0
        const rtoRev = parseFloat(nc.rto_rev) || 0
        const returnRev = parseFloat(nc.return_rev) || 0
        const cirRev = parseFloat(nc.cir_rev) || 0
        const gar = gross - cancelRev - rtoRev - returnRev - cirRev
        const gstRatio = gross > 0 ? (gross - excRev) / gross : 0
        const netCalc = { gross, excRev, cancelRev, rtoRev, returnRev, cirRev, netRev: gar * (1 - gstRatio) }
        ...
        return {
          ...
          catMap: Object.fromEntries((r.eboCategory || []).map(x => [x.Category || 'Unknown', { rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: { size: parseInt(x.orders)||0 }, units: parseInt(x.units)||0, aspUnits: parseInt(x.asp_units)||parseInt(x.units)||0, cancelled: parseInt(x.cancelled)||0, rto: parseInt(x.rto)||0, cir: parseInt(x.cir)||0, exch: parseInt(x.exch)||0, cancelRev: parseFloat(x.cancel_rev)||0, rtoRev: parseFloat(x.rto_rev)||0, cirRev: parseFloat(x.cir_rev)||0, exchRev: parseFloat(x.exch_rev)||0 }])),
          catPrevMap: Object.fromEntries((r.eboCategoryPrev || []).map(x => [x.Category || 'Unknown', parseFloat(x.rev)||0])),
          subCatMap: Object.fromEntries((r.eboSubCategory || []).map(x => [`${x.Category||'Unknown'}::${x.SubCategory||'Unknown'}`, { rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: { size: parseInt(x.orders)||0 }, units: parseInt(x.units)||0, aspUnits: parseInt(x.asp_units)||parseInt(x.units)||0, cancelled: parseInt(x.cancelled)||0, rto: parseInt(x.rto)||0, cir: parseInt(x.cir)||0, exch: parseInt(x.exch)||0, cancelRev: parseFloat(x.cancel_rev)||0, rtoRev: parseFloat(x.rto_rev)||0, cirRev: parseFloat(x.cir_rev)||0, exchRev: parseFloat(x.exch_rev)||0 }])),
          subCatPrevMap: Object.fromEntries((r.eboSubCategoryPrev || []).map(x => [`${x.Category||'Unknown'}::${x.SubCategory||'Unknown'}`, parseFloat(x.rev)||0])),
          skuMap: (() => { const m = {}; (r.eboSKU || []).forEach(x => { const cat = x.Category||'Unknown', sc = x.SubCategory||'Unknown', sku = x.sku; if (!m[cat]) m[cat] = {}; if (!m[cat][sc]) m[cat][sc] = {}; m[cat][sc][sku] = { rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, cancelled: parseInt(x.cancelled)||0, rto: parseInt(x.rto)||0, cir: parseInt(x.cir)||0, exch: parseInt(x.exch)||0, cancelRev: parseFloat(x.cancel_rev)||0, rtoRev: parseFloat(x.rto_rev)||0, cirRev: parseFloat(x.cir_rev)||0, exchRev: parseFloat(x.exch_rev)||0 } }); return m })(),
          stateMap: Object.fromEntries((r.eboState || []).filter(x => x.state).map(x => [x.state, { rev: parseFloat(x.rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, cities: { size: parseInt(x.cities)||0 }, rtoOrders: parseInt(x.rto_orders)||0 }])),
          ...
          cityRows: (r.eboCity || []).map(x => ({ city: x.city, state: x.state || '', region: x.region || '', orders: parseInt(x.orders)||0, rev: parseFloat(x.rev)||0, rtoOrders: parseInt(x.rto_orders)||0 })).filter(x => x.city),
      ...
      amzSC: {
        ...
        netCalc: (() => { const row = r.amzSCNetCalc?.[0] || {}; const gross = parseFloat(row.gross_rev)||0; const grossExc = parseFloat(row.gross_exc_rev)||0; const returnRev = parseFloat(row.return_rev)||0; const cancelRev = parseFloat(row.cancel_rev)||0; const gstRatio = gross > 0 ? (gross - grossExc) / gross : 0; const afterReturns = Math.max(gross - cancelRev - returnRev, 0); const netRev = afterReturns * (1 - gstRatio); return { gross, cancelRev, returnRev, afterReturns, netRev, gstRatio } })(),
      ...
      flipkart: {
        ...
        netCalc: (() => {
          ...
          const gstRatio = all.rev > 0 ? (all.rev - all.excRev) / all.rev : 0
          const afterDeductions = Math.max(all.rev - all.cancelRev - all.returnRev, 0)
          return { gross: all.rev, cancelRev: all.cancelRev, returnRev: all.returnRev, netRev: afterDeductions * (1 - gstRatio), gstRatio }
        })(),
      ...
      offline: {
        ...
        categoryRows: (r.offlineCategory || []).map(x => ({ subChannel: x.SubChannel, category: x.Category || 'Unknown', rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, cnRev: parseFloat(x.cn_rev)||0 })),
        subCategoryRows: (r.offlineSubCategory || []).map(x => ({ subChannel: x.SubChannel, category: x.Category || 'Unknown', subCategory: x.SubCategory || 'Unknown', rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0 })),
        skuRows: (r.offlineSKU || []).map(x => ({ subChannel: x.SubChannel, category: x.Category || 'Unknown', subCategory: x.SubCategory || 'Unknown', sku: x.sku, rev: parseFloat(x.rev)||0, excRev: parseFloat(x.exc_rev)||0, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0 })),
      ...
      ads: {
        ...
        prevCategoryBreakdown: (r.prevAdsCategoryBreakdown || []).map(x => ({ platform: x.platform, targetType: x.target_type, category: x.category || 'Unknown', subCategory: x.target_type === 'product' ? x.product_name : null, spend: parseFloat(x.spend)||0, clicks: parseFloat(x.clicks)||0 })),
