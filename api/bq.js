...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    amzSCReturnRate: `WITH q AS (${base}) SELECT ROUND(SUM(CASE WHEN Order_Status NOT IN ('Return','RTO','CIR') AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Inc_GST ELSE 0 END), 2) AS total_rev_inc, ROUND(SUM(CASE WHEN Order_Status IN ('Return','RTO','CIR') THEN ABS(SellingPrice_Inc_GST) ELSE 0 END), 2) AS returned_rev FROM q WHERE SubChannel = 'Amazon Seller Central'`,
