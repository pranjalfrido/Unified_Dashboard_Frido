...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shState: `WITH q AS (${base}) SELECT UPPER(TRIM(State)) AS state, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, COUNT(DISTINCT City) AS cities, COUNT(DISTINCT CASE WHEN Order_Status='RTO' THEN OrderId END) AS rto_orders FROM q WHERE Channel='Shopify' AND State IS NOT NULL AND TRIM(State) != '' GROUP BY UPPER(TRIM(State)) ORDER BY rev DESC LIMIT 30`,
    ...
    shPaymentTypes: `WITH q AS (${base}) SELECT PaymentMode AS payment_type, COUNT(DISTINCT OrderId) AS orders FROM q WHERE Channel = 'Shopify' AND PaymentMode IS NOT NULL AND TRIM(PaymentMode) != '' GROUP BY PaymentMode ORDER BY orders DESC LIMIT 50`,
