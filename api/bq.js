...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    adsCategoryBreakdown: `WITH im_sub AS (SELECT LOWER(TRIM(Sub_category)) AS sub_key, ANY_VALUE(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE Sub_category IS NOT NULL GROUP BY sub_key), im_cat AS (SELECT LOWER(TRIM(Category_Name)) AS cat_key, ANY_VALUE(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE Category_Name IS NOT NULL GROUP BY cat_key) SELECT a.platform, a.target_type, a.product_name, CASE WHEN a.target_type = 'product' THEN TRIM(COALESCE(MAX(i_prod.Category_Name), 'Unknown')) WHEN a.target_type = 'category' THEN TRIM(COALESCE(MAX(i_cat.Category_Name), a.product_name)) END AS category, ROUND(SUM(a.spend),0) AS spend, ROUND(SUM(a.orders),0) AS orders, ROUND(SUM(a.revenue),0) AS revenue, ROUND(SUM(a.clicks),0) AS clicks, ROUND(SUM(a.impressions),0) AS impressions, ROUND(SAFE_DIVIDE(SUM(a.clicks),SUM(a.impressions))*100,2) AS ctr, ROUND(SAFE_DIVIDE(SUM(a.spend),SUM(a.clicks)),2) AS cpc, ROUND(SAFE_DIVIDE(SUM(a.revenue),SUM(a.spend)),2) AS roas FROM \`frido-429506.production.fact_all_platform_ads_report\` a LEFT JOIN im_sub i_prod ON a.target_type = 'product' AND LOWER(TRIM(a.product_name)) = i_prod.sub_key LEFT JOIN im_cat i_cat ON a.target_type = 'category' AND LOWER(TRIM(a.product_name)) = i_cat.cat_key WHERE a.report_date BETWEEN '${start}' AND '${end}' AND a.target_type != 'all' AND a.product_name IS NOT NULL AND TRIM(a.product_name) != '' GROUP BY a.platform, a.target_type, a.product_name ORDER BY a.platform, spend DESC LIMIT 500`,
