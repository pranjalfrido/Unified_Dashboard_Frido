...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    amzSCReturnRate: `WITH latest_settle AS (SELECT MAX(CAST(settlement_end_date AS DATE)) AS max_dt FROM \`frido-429506.production.fact_all_settlement_report\` WHERE settlement_region = 'India'), cutoff AS (SELECT DATE_SUB(max_dt, INTERVAL 15 DAY) AS cutoff_dt FROM latest_settle), roll_start AS (SELECT DATE_SUB(max_dt, INTERVAL 30 DAY) AS roll_start_dt FROM latest_settle), sales AS (SELECT s.OrderId, SUM(s.SellingPrice_Inc_GST) AS order_rev FROM \`frido-429506.production.fact_all_platform_sales_report\` s, roll_start, cutoff WHERE s.SubChannel = 'Amazon Seller Central' AND s.FulfilmentStatus != 'Cancelled' AND s.SellingPrice_Inc_GST > 0 AND CAST(s.OrderDate AS DATE) BETWEEN roll_start.roll_start_dt AND cutoff.cutoff_dt GROUP BY s.OrderId), ret AS (SELECT DISTINCT order_id FROM \`frido-429506.production.fact_all_settlement_report\` WHERE transaction_type = 'Refund' AND settlement_region = 'India') SELECT ROUND(SUM(s.order_rev), 2) AS total_rev_inc, ROUND(SUM(CASE WHEN ret.order_id IS NOT NULL THEN s.order_rev ELSE 0 END), 2) AS returned_rev FROM sales s LEFT JOIN ret ON s.OrderId = ret.order_id`,
