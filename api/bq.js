...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    adsZeroOrder: `SELECT platform, COALESCE(NULLIF(TRIM(product_name),''), NULLIF(TRIM(campaign_name),''), 'Unknown') AS product, campaign_name, ROUND(SUM(spend),0) AS spend, ROUND(SUM(orders),0) AS orders FROM \`frido-429506.production.fact_all_platform_ads_report\` WHERE report_date BETWEEN '${start}' AND '${end}' AND platform IN ('Google','Flipkart') GROUP BY platform, product, campaign_name HAVING SUM(spend) > 0 ORDER BY platform, spend DESC LIMIT 500`,
  ...
  try {
    ...
    const payload = {
      ...
      ads: {
        ...
        zeroOrder: (r.adsZeroOrder || []).map(x => ({ platform: x.platform, product: x.product, campaign: x.campaign_name, spend: parseFloat(x.spend)||0, orders: parseFloat(x.orders)||0 })),
