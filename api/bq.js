...
ORDER BY order_date`,
    ...
    offlineCity: `WITH q AS (${base}) SELECT SubChannel, INITCAP(TRIM(City)) AS city, INITCAP(TRIM(State)) AS state, MAX(Region) AS region, COUNT(DISTINCT CASE WHEN Order_Status='Sales' THEN OrderId END) AS orders, SUM(CASE WHEN Order_Status='Sales' THEN SellingPrice_Inc_GST ELSE 0 END) AS rev FROM q WHERE Channel='offline_sales' AND City IS NOT NULL AND TRIM(City) != '' GROUP BY SubChannel, INITCAP(TRIM(City)), INITCAP(TRIM(State)) ORDER BY rev DESC LIMIT 200`,
  ...
  try {
    ...
    const payload = {
      ...
      offline: {
        totalsBySub: (r.offlineTotals || []).map(x => ({ subChannel: x.SubChannel, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, revSales: parseFloat(x.rev_sales)||0, excRevSales: parseFloat(x.exc_rev_sales)||0, cnRev: parseFloat(x.cn_rev)||0, cnExcRev: parseFloat(x.cn_exc_rev)||0 })),
        prevBySub: (r.prevOffline || []).map(x => ({ subChannel: x.SubChannel, orders: parseInt(x.orders)||0, units: parseInt(x.units)||0, revSales: parseFloat(x.rev_sales)||0, excRevSales: parseFloat(x.exc_rev_sales)||0, cnRev: parseFloat(x.cn_rev)||0, cnExcRev: parseFloat(x.cn_exc_rev)||0 })),
