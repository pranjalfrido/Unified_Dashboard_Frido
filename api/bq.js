...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shCategory: `WITH q AS (${base}) SELECT Category, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, SUM(ItemQty) AS units, COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancelled, COUNT(DISTINCT CASE WHEN Order_Status='RTO' THEN OrderId END) AS rto, COUNT(DISTINCT CASE WHEN Order_Status='CIR' THEN OrderId END) AS cir, COUNT(DISTINCT CASE WHEN Order_Status='Exchange' THEN OrderId END) AS exch, SUM(CASE WHEN Order_Status='Cancelled' THEN SellingPrice_Inc_GST ELSE 0 END) AS cancel_rev, SUM(CASE WHEN Order_Status='RTO' THEN SellingPrice_Inc_GST ELSE 0 END) AS rto_rev, SUM(CASE WHEN Order_Status='CIR' THEN SellingPrice_Inc_GST ELSE 0 END) AS cir_rev, SUM(CASE WHEN Order_Status='Exchange' THEN SellingPrice_Inc_GST ELSE 0 END) AS exch_rev FROM q WHERE Channel='Shopify' AND (SubChannel != 'Shopify B2B' OR SubChannel IS NULL) GROUP BY Category ORDER BY rev DESC`,
    ...
    byOrderStatus: `WITH q AS (${base}) SELECT Order_Status AS order_status, COUNT(DISTINCT OrderId) AS cnt, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel = 'Shopify' AND (SubChannel != 'Shopify B2B' OR SubChannel IS NULL) AND Country = 'India' GROUP BY Order_Status`,
    ...
    byDailyReturnTrend: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, COUNT(DISTINCT OrderId) AS total_orders, COUNTIF(is_rto = 1) AS rto_orders, COUNTIF(is_exchange = 1) AS exch_orders, COUNTIF(is_CIR_return = 1) AS cir_orders, COUNT(DISTINCT CASE WHEN Order_Status='Cancelled' THEN OrderId END) AS cancel_orders FROM q WHERE Channel = 'Shopify' AND (SubChannel != 'Shopify B2B' OR SubChannel IS NULL) GROUP BY date ORDER BY date`,
    ...
    byExchange: `WITH q AS (${base}) SELECT COUNT(DISTINCT OrderId) AS exchange_orders FROM q WHERE is_exchange = 1 AND Channel = 'Shopify' AND (SubChannel != 'Shopify B2B' OR SubChannel IS NULL)`,
    byRTO: `WITH q AS (${base}) SELECT SUM(SellingPrice_Inc_GST) AS rto_rev, COUNT(DISTINCT OrderId) AS rto_orders FROM q WHERE is_rto = 1 AND Channel = 'Shopify' AND (SubChannel != 'Shopify B2B' OR SubChannel IS NULL)`,
    ...
    amzSCReturnRate: `WITH q AS (${base}) SELECT ROUND(SUM(CASE WHEN FulfilmentStatus != 'Cancelled' AND SellingPrice_Inc_GST > 0 THEN SellingPrice_Inc_GST ELSE 0 END), 2) AS total_rev_inc, ROUND(SUM(CASE WHEN Order_Status IN ('Return','RTO','CIR') THEN ABS(SellingPrice_Inc_GST) ELSE 0 END), 2) AS returned_rev FROM q WHERE SubChannel = 'Amazon Seller Central'`,
    ...
    fkStates: `WITH q AS (${fkBase}) SELECT COALESCE(NULLIF(TRIM(UPPER(State)),'-'), 'OTHERS') AS state, CASE WHEN SubChannel='Flipkart FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT OrderId) AS orders, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev, COUNT(DISTINCT CASE WHEN FulfilmentStatus='Returned' OR Order_Status='RTO' THEN OrderId END) AS rto_orders FROM q WHERE Channel='Flipkart' GROUP BY state, sub ORDER BY rev DESC`,
    fkCities: `WITH q AS (${fkBase}) SELECT UPPER(TRIM(City_L2)) AS city, UPPER(TRIM(State)) AS state, CASE WHEN SubChannel='Flipkart FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT OrderId) AS orders, ROUND(SUM(SellingPrice_Inc_GST),0) AS rev, COUNT(DISTINCT CASE WHEN FulfilmentStatus='Returned' OR Order_Status='RTO' THEN OrderId END) AS rto_orders FROM q WHERE Channel='Flipkart' AND City_L2 IS NOT NULL AND TRIM(City_L2) != '' GROUP BY city, state, sub ORDER BY rev DESC LIMIT 100`,
    ...
    fkStateTotal: `WITH q AS (${fkBase}) SELECT CASE WHEN SubChannel='Flipkart FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, ROUND(SUM(SellingPrice_Inc_GST),0) AS total_rev FROM q WHERE Channel='Flipkart' GROUP BY sub`,
    fkCityTotal: `WITH q AS (${fkBase}) SELECT CASE WHEN SubChannel='Flipkart FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, ROUND(SUM(SellingPrice_Inc_GST),0) AS total_rev FROM q WHERE Channel='Flipkart' AND City_L2 IS NOT NULL AND TRIM(City_L2) != '' GROUP BY sub`,
  ...
  try {
    ...
    const payload = {
      ...
      flipkart: {
        ...
        states: (r.fkStates || []).map(x => ({ state: (!x.state || x.state.trim() === '-' || x.state.trim() === '') ? 'Others' : x.state, sub: x.sub, orders: parseInt(x.orders)||0, rev: parseFloat(x.rev)||0, rtoOrders: parseInt(x.rto_orders)||0 })),
        cities: (r.fkCities || []).map(x => ({ city: x.city, state: x.state, sub: x.sub, orders: parseInt(x.orders)||0, rev: parseFloat(x.rev)||0, rtoOrders: parseInt(x.rto_orders)||0 })),
