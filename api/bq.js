...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    amzVCASINs: `WITH q AS (${base}) SELECT ProductId AS asin, ChannelSKUCode AS sku, SUM(ItemQty) AS ordered_units, ROUND(SUM(SellingPrice_Inc_GST),0) AS ordered_rev, SUM(ItemQty) AS shipped_units, 0 AS returns FROM q WHERE SubChannel = 'Amazon Vendor Central' GROUP BY asin, sku ORDER BY ordered_rev DESC LIMIT 20`,
    ...
    fkTotals: `SELECT CASE WHEN UPPER(TRIM(fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT order_id) AS orders, ROUND(SUM(CAST(price_after_discount_price_before_discount_total_discountx AS FLOAT64)),0) AS rev, ROUND(SUM(CAST(taxable_value_final_invoice_amount_taxesx AS FLOAT64)),0) AS exc_rev, SUM(CAST(CAST(item_quantity AS FLOAT64) AS INT64)) AS units FROM \`frido-429506.flipkart_reports.sales_report\` WHERE DATE(SUBSTR(order_date,1,10)) BETWEEN '${start}' AND '${end}' GROUP BY sub`,
    fkDaily: `SELECT CAST(DATE(SUBSTR(order_date,1,10)) AS STRING) AS date, CASE WHEN UPPER(TRIM(fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT order_id) AS orders, ROUND(SUM(CAST(price_after_discount_price_before_discount_total_discountx AS FLOAT64)),0) AS rev FROM \`frido-429506.flipkart_reports.sales_report\` WHERE DATE(SUBSTR(order_date,1,10)) BETWEEN '${start}' AND '${end}' GROUP BY date, sub ORDER BY date`,
    fkStatus: `SELECT event_sub_type AS status, CASE WHEN UPPER(TRIM(fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT order_id) AS orders FROM \`frido-429506.flipkart_reports.sales_report\` WHERE DATE(SUBSTR(order_date,1,10)) BETWEEN '${start}' AND '${end}' AND event_sub_type IS NOT NULL GROUP BY status, sub ORDER BY orders DESC`,
    fkSKUs: `SELECT TRIM(REPLACE(REGEXP_REPLACE(sku,r'"{2,}SKU:-*"{0,}',''),'"','')) AS sku, CASE WHEN UPPER(TRIM(fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT order_id) AS orders, SUM(CAST(CAST(item_quantity AS FLOAT64) AS INT64)) AS units, ROUND(SUM(CAST(price_after_discount_price_before_discount_total_discountx AS FLOAT64)),0) AS rev FROM \`frido-429506.flipkart_reports.sales_report\` WHERE DATE(SUBSTR(order_date,1,10)) BETWEEN '${start}' AND '${end}' AND sku IS NOT NULL GROUP BY sku, sub ORDER BY rev DESC LIMIT 30`,
    fkCategories: `SELECT COALESCE(im.Category_Name,'Unknown') AS category, CASE WHEN UPPER(TRIM(f.fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT f.order_id) AS orders, ROUND(SUM(CAST(f.price_after_discount_price_before_discount_total_discountx AS FLOAT64)),0) AS rev, SUM(CAST(CAST(f.item_quantity AS FLOAT64) AS INT64)) AS units FROM \`frido-429506.flipkart_reports.sales_report\` f LEFT JOIN (SELECT DISTINCT TRIM(Product_Code) AS Product_Code, TRIM(Category_Name) AS Category_Name FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__frido_item_sku_master\` WHERE TRIM(Product_Code)!='') im ON TRIM(REPLACE(REGEXP_REPLACE(f.sku,r'"{2,}SKU:-*"{0,}',''),'"',''))=im.Product_Code WHERE DATE(SUBSTR(f.order_date,1,10)) BETWEEN '${start}' AND '${end}' GROUP BY category, sub ORDER BY rev DESC`,
    fkStates: `SELECT UPPER(TRIM(customer_s_delivery_state)) AS state, CASE WHEN UPPER(TRIM(fulfilment_type))='FBF' THEN 'FBF' ELSE 'NON-FBF' END AS sub, COUNT(DISTINCT order_id) AS orders, ROUND(SUM(CAST(price_after_discount_price_before_discount_total_discountx AS FLOAT64)),0) AS rev FROM \`frido-429506.flipkart_reports.sales_report\` WHERE DATE(SUBSTR(order_date,1,10)) BETWEEN '${start}' AND '${end}' AND customer_s_delivery_state IS NOT NULL GROUP BY state, sub ORDER BY rev DESC`,
  }
