...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shDaily: `WITH q AS (${base}) SELECT CAST(OrderDate AS STRING) AS date, SUM(SellingPrice_Inc_GST) AS rev, SUM(SellingPrice_Exc_GST) AS exc_rev, COUNT(DISTINCT OrderId) AS orders, SUM(ItemQty) AS units FROM q WHERE Channel='Shopify' AND (SubChannel != 'Shopify B2B' OR SubChannel IS NULL) GROUP BY date ORDER BY date`,
