...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    shCity: `WITH q AS (${base}) SELECT City_L3 AS city, UPPER(TRIM(State)) AS state, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE Channel='Shopify' AND City_L3 IS NOT NULL AND TRIM(City_L3) != '' GROUP BY City_L3, UPPER(TRIM(State)) ORDER BY rev DESC LIMIT 50`,
    ...
    byCity: `WITH q AS (${base}) SELECT City_L3 AS city, UPPER(TRIM(State)) AS state, Region AS region, City_Tier AS city_tier, COUNT(DISTINCT OrderId) AS orders, SUM(SellingPrice_Inc_GST) AS rev FROM q WHERE City_L3 IS NOT NULL AND TRIM(City_L3) != '' GROUP BY City_L3, UPPER(TRIM(State)), Region, City_Tier ORDER BY rev DESC LIMIT 50`,
