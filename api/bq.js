...
export default async function handler(req, res) {
  ...
  const queries = {
    ...
    blTotals: `WITH deduped AS (SELECT item_id, item_name, city_id, city_name, category, date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS mrp FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE date BETWEEN '${start}' AND '${end}') WHERE rn=1) SELECT SUM(qty_sold) AS units, ROUND(SUM(mrp*qty_sold),0) AS rev, COUNT(DISTINCT item_id) AS skus, COUNT(DISTINCT city_name) AS cities, COUNT(DISTINCT date) AS days FROM deduped`,
    blDaily: `WITH deduped AS (SELECT item_id, city_id, date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS mrp FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE date BETWEEN '${start}' AND '${end}') WHERE rn=1) SELECT CAST(date AS STRING) AS date, SUM(qty_sold) AS units, ROUND(SUM(mrp*qty_sold),0) AS rev FROM deduped GROUP BY date ORDER BY date`,
    blCategories: `WITH deduped AS (SELECT item_id, city_id, category, date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS mrp FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE date BETWEEN '${start}' AND '${end}') WHERE rn=1) SELECT category, SUM(qty_sold) AS units, ROUND(SUM(mrp*qty_sold),0) AS rev, COUNT(DISTINCT item_id) AS skus FROM deduped GROUP BY category ORDER BY rev DESC`,
    blSKUs: `WITH deduped AS (SELECT item_id, item_name, city_name, date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS mrp FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE date BETWEEN '${start}' AND '${end}') WHERE rn=1) SELECT item_id, item_name, SUM(qty_sold) AS units, ROUND(SUM(mrp*qty_sold),0) AS rev, MAX(mrp) AS mrp, COUNT(DISTINCT city_name) AS cities FROM deduped GROUP BY item_id, item_name ORDER BY rev DESC`,
    blCities: `WITH deduped AS (SELECT item_id, city_name, date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS mrp FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE date BETWEEN '${start}' AND '${end}') WHERE rn=1) SELECT city_name, SUM(qty_sold) AS units, ROUND(SUM(mrp*qty_sold),0) AS rev, COUNT(DISTINCT item_id) AS skus FROM deduped GROUP BY city_name ORDER BY rev DESC`,
  }
