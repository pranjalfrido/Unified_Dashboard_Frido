...
price_blinkit AS (SELECT DISTINCT TRIM(productid) AS productid, CAST(NULL AS FLOAT64) AS selling_price FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__productid_sku_mapping\` WHERE TRIM(channelname) = 'Blinkit'),
price_zepto AS (SELECT DISTINCT TRIM(productid) AS productid, CAST(NULL AS FLOAT64) AS selling_price FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__productid_sku_mapping\` WHERE TRIM(channelname) = 'Zepto'),
price_instamart AS (SELECT DISTINCT TRIM(productid) AS productid, CAST(NULL AS FLOAT64) AS selling_price FROM \`frido-429506.sharepoint_to_gcp.Frido_Item_Master__productid_sku_mapping\` WHERE TRIM(channelname) = 'Instamart'),
...
pincode_master AS (SELECT DISTINCT CAST(Pincode AS STRING) AS pincode, City_L1, State, Region, City_Tier, Tier_Label, Is_Metro_City FROM \`frido-429506.production.pincode_city_master\`),
city_name_master AS (SELECT DISTINCT City_L1, State, Region, City_Tier, Tier_Label, Is_Metro_City FROM \`frido-429506.production.pincode_city_master\`),
...
blinkit_raw AS (SELECT item_id, city_name, DATE(date) AS date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS total_mrp, SAFE_DIVIDE(CAST(mrp AS FLOAT64), CAST(qty_sold AS FLOAT64)) AS mrp_per_unit, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE DATE(date) BETWEEN '${s}' AND '${e}'),
blinkit AS (SELECT 'India' AS Country, CONCAT('BLI', LPAD(CAST(ROW_NUMBER() OVER (ORDER BY b.date, b.item_id) AS STRING), 6, '0')) AS OrderId, 'Blinkit' AS Channel, 'Blinkit' AS SubChannel, 'Blinkit' AS ChannelAccount, b.date AS OrderDate, CAST(NULL AS STRING) AS State, b.city_name AS City, CAST(NULL AS STRING) AS Pincode, b.item_id AS ProductId, b.item_id AS ChannelSKUCode, b.qty_sold AS ItemQty, b.total_mrp AS SellingPrice_Inc_GST, SAFE_MULTIPLY(CAST(pb.selling_price AS FLOAT64), b.qty_sold) AS SellingPrice_Exc_GST, CAST(NULL AS STRING) AS OrderTrackingStatus, CAST(NULL AS STRING) AS FulfilmentStatus, CAST(NULL AS STRING) AS FinancialStatus, CAST(NULL AS FLOAT64) AS Tax, CAST(NULL AS STRING) AS fulfillment_channel, CAST(NULL AS STRING) AS RefundStatus, CAST(NULL AS STRING) AS PaymentMode, CAST(NULL AS STRING) AS CustomerId, CAST(NULL AS STRING) AS voucher_code FROM blinkit_raw b LEFT JOIN price_blinkit pb ON TRIM(b.item_id) = pb.productid WHERE b.rn = 1),
