...
blinkit_raw AS (SELECT item_id, city_name, date, SUM(CAST(qty_sold AS FLOAT64)) AS qty_sold, MAX(CAST(mrp AS FLOAT64)) AS mrp FROM \`frido-429506.production.fact_blinkit_sales_report\` WHERE date BETWEEN '${s}' AND '${e}' GROUP BY item_id, city_name, date),
blinkit AS (SELECT 'India' AS Country, CONCAT('BLI', LPAD(CAST(ROW_NUMBER() OVER (ORDER BY b.date, b.item_id) AS STRING), 6, '0')) AS OrderId, 'Blinkit' AS Channel, 'Blinkit' AS SubChannel, 'Blinkit' AS ChannelAccount, b.date AS OrderDate, CAST(NULL AS STRING) AS State, b.city_name AS City, CAST(NULL AS STRING) AS Pincode, b.item_id AS ProductId, b.item_id AS ChannelSKUCode, b.qty_sold AS ItemQty, b.mrp AS SellingPrice_Inc_GST, CAST(pb.selling_price AS FLOAT64) AS SellingPrice_Exc_GST, CAST(NULL AS STRING) AS OrderTrackingStatus, CAST(NULL AS STRING) AS FulfilmentStatus, CAST(NULL AS STRING) AS FinancialStatus, CAST(NULL AS FLOAT64) AS Tax, CAST(NULL AS STRING) AS fulfillment_channel, CAST(NULL AS STRING) AS RefundStatus, CAST(NULL AS STRING) AS PaymentMode, CAST(NULL AS STRING) AS CustomerId, CAST(NULL AS STRING) AS voucher_code FROM blinkit_raw b LEFT JOIN price_blinkit pb ON TRIM(CAST(b.item_id AS STRING)) = pb.productid),
