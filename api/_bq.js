...
blinkit_raw AS (SELECT item_id, city_name, DATE(date) AS date, CAST(qty_sold AS FLOAT64) AS qty_sold, CAST(mrp AS FLOAT64) AS mrp, ROW_NUMBER() OVER (PARTITION BY item_id, city_id, date ORDER BY _dlt_load_id DESC) AS rn FROM \`frido-429506.partnerbizz_reports_v2.sales\` WHERE DATE(date) BETWEEN '${s}' AND '${e}'),
