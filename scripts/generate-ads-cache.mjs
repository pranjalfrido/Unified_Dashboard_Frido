...
const queries = {
  ...
  shopifyNewCustsDaily: `WITH orders AS (SELECT CAST(order_date_ist AS STRING) AS date, customer_id, MIN(order_date_ist) OVER (PARTITION BY customer_id) AS first_order_date FROM \`frido-429506.production.fact_shopify_myfrido_mobility_all_orders\` WHERE order_date_ist BETWEEN '${start}' AND '${end}' AND customer_id IS NOT NULL) SELECT date, COUNT(DISTINCT customer_id) AS n_custs, COUNTIF(CAST(first_order_date AS STRING) = date) AS new_custs FROM orders GROUP BY date ORDER BY date`,
...
const payload = {
  ...
  ads: {
    ...
    shopifyNewCustsDaily: (r.shopifyNewCustsDaily || []).map(x => ({ date: x.date, nCusts: i(x.n_custs), newCusts: i(x.new_custs) })),
